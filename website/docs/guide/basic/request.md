...
> 结合 [状态管理文档](/guide/basic/store.md) 使用
...
项目开发初期，后端接口可能还没开发好或不够稳定，此时前端可以通过 Mock 的方式来模拟接口，参考文档 [本地 Mock 能力](/guide/advanced/mock.md)。

### 使用真实的后端接口调试前端代码

当项目开发到一定时间段时，我们需要联调后端接口，此时可能会遇到各种跨域问题，参考文档 [本地 Proxy 能力](/guide/advanced/proxy.md)。
...
结合[环境配置](/guide/basic/config.md)即可实现不同环境使用不同的 baseURL：
