...
import { REG_POST } from './constants.js';
...
export default function (this: webpack.LoaderContext<any>) {
  ...
  const { blended } = options;
  const { pxTransformConfig } = options;
  const { loaders } = this;
  const thisLoaderIndex = loaders.findIndex(item => normalizePath(item.path).indexOf('@ice/miniapp-loader') >= 0);
